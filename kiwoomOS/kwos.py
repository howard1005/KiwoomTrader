...
class KiwoomOS:
    ...
    def _tr_request_task(self, task):
        ...
        print("[LOG] [_tr_request_task] request {}".format(request))
    ...
    def _api_onReceiveTrData(self, sScrNo, sRQName, sTrcode, sRecordName, sPrevNext, nDataLength, sErrorCode,
                             sMessage, sSplmMsg):
        print("[LOG] [_api_onReceiveTrData] sRQName:{} sTrcode:{}".format(sRQName, sTrcode))
        ...
        if sTrcode in tr_data:
            trData = {}
            for k, v in tr_data[sTrcode].items():
                trData[k] = self.kiwoom.dynamicCall("GetCommData(QString, QString, int, QString)", sTrcode, sRQName, 0, k).strip()
            for event_func in self._onReceiveTr_observer:
                event_func(sRQName, sTrcode, sPrevNext, trData)
    ...
    def _api_onReceiveRealData(self, sCode, sRealType, sRealData):
        ...
        if sRealType == '주식체결':
            ...
            '''
        # print("[kwos] [_api_onReceiveRealData] sRealType : {}\n".format(sRealType))
        if sRealType in real_data:
            realData = {}
            for k, v in real_data[sRealType].items():
                realData[k] = self.kiwoom.dynamicCall("GetCommRealData(QString, int)", sCode, v)
            '''
            sRealData_list = sRealData.split('\t')
            print("[kwos] [_api_onReceiveRealData] sRealData_list : {}\n".format(sRealData_list))
            for i,k in enumerate(real_data[sRealType]):
                if i<len(sRealData_list):
                    realData[k] = sRealData_list[i]
                else:
                    realData[k] = "{}".format(i)
            '''
            for event_func in self._onReceiveReal_observer:
                event_func(sCode, sRealType, realData)

        '''
        else:
            for event_func in self._onReceiveRealExt_observer:
                event_func(sCode, sRealType, sRealType)
        '''
    ...
    def requestTr(self, rqName, trCode, prevNext=0):
        ...
        print("[kwos] [requestTr] task", task)
        self.inputValueList = []
    ...
    def addRealData(self, codeList):
        print("[kwos] [addRealData] codeList : {}".format(codeList))
